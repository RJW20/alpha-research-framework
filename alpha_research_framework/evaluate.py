...
def evaluate(
    universe: Universe, alphas: list[Alpha], metrics: set[str] = {"ic"}
    ...
    for metric in metrics:
        result[metric] = Metric.from_id(metric).dataframe(
            index=universe.dates,
            base_columns=tuples,
            base_column_names=names
        )
    ...
    for t, date in enumerate(universe.dates):
        ...
        for alpha in alphas:
            ...
            for horizon in horizons_to_evaluate:
                for metric in metrics:
                    df = result[metric]
                    df.loc[date, pd.IndexSlice[alpha.NAME, horizon, :]] = (
                        Metric.from_id(metric).compute(signal, fut_ret[horizon])
                    )
